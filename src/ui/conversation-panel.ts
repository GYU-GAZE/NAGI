...
export class ConversationPanel {
 ...
 render(container:HTMLElement,kind:string){
  ...
  const list=new VirtualList<SearchResult>(result=>{const row=el('div');row.append(button(`${result.message.role==='user'?'Você':'Resposta'} · ${result.snippet}`,()=>void this.navigator.jump(result.message.id,undefined,query.value)),button('Anotar / foco',()=>edit(result.message.id)));return row;},90);
